...
from icra_tools import statistics, utils, learner, knet, net
from icra_tools.expert import load_policy
from icra_tools.supervisor import NetSupervisor, Supervisor
...
def main():
    ...
    opt.sess = None
    opt.misc = Options()
    opt.pi = net.Network([64, 64], .01, 400)
    opt.pi.load_weights('meta/weights.txt', 'meta/stats.txt')
    ...
    for i in range(opt.iters):
        ...
        if (i + 1) % (opt.iters/opt.misc.num_evaluations) == 0:
            ...
            for j in range(opt.samples):
                ...
                # states_test, int_actions_test, _, r_test, _, lnr_score = statistics.collect_score_traj_multiple(opt.env, lnr, ocs, opt.t, False, False, True)
                # states_robust, int_actions_robust, _, r_robust, freq, rob_score, mags = statistics.collect_robust_traj_multiple(opt.env, lnr, ocs, opt.t, opt, False, False, True)
                states_test, int_actions_test, _, r_test, = statistics.collect_traj(opt.env, lnr, opt.t, False, False, True)
