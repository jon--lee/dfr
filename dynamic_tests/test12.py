    ...
    for i in range(opt.iters):
        ...
        if (i + 1) % (opt.iters/opt.misc.num_evaluations) == 0:
            ...
            for j in range(opt.samples):
                ...
                states_robust, int_actions_robust, _, r_robust, freq, rob_score, mags = statistics.collect_robust_traj_multiple(opt.env, lnr, ocs, opt.t, False, False)
            ...
            if index == (opt.misc.num_evaluations - 1):
                make_bar_graphs(ocs, trajs_train, trajs_valid, trajs_test, opt)
