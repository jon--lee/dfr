...
from icra_tools import statistics3 as stats
    ...
    for i in range(opt.iters):
        ...
        if (i + 1) % (opt.iters/opt.misc.num_evaluations) == 0:
            ...
            for j in range(opt.samples):
                ...
                states_robust, int_actions_robust, _, r_robust, freq, rob_score, mags = stats.collect_robust_traj_multiple(opt.env, opt.sim, lnr, ocs, opt.t, opt, False, False)
