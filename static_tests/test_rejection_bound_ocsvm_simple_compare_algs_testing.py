...
def run_trial(opt):
    ...
    for i in range(opt.misc.samples):
        ...
        while not triggered:
            ...
            results = rec_statistics.collect_rec_random(opt.env, lnr, ocs, opt.t, opt, max_KLs, visualize=False, early_stop=False, init_state=init_state, max_rec=max_rec)
    ...
    approx_grad_scores = np.zeros((1, max_rec + 1))
    approx_grad_cutoffs = np.zeros((1, max_rec + 1))
    ...
    for i in range(1):
        ...
        while not triggered:
            print "\t\tNot yet triggered"
            results = rec_statistics.collect_rec_approx_grad(opt.env, lnr, ocs, opt.t, opt, max_KLs, visualize=False, early_stop=False, init_state=init_state, max_rec=max_rec)
            triggered = results[-3]['triggered']
