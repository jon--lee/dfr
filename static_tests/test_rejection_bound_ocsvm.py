...
def run_trial(opt):
    ocs = [ svm.OneClassSVM(kernel='rbf', gamma = .1, nu = .03) for t in range(opt.t) ]
    ...
    if total_failures > 0:
        print "Fails in support: " + str(float(total_failures_in_support) / total_failures)
    print total_failures_in_support
    print total_failures

    print str(samples_failed_in_support) + " failed in support"
    print "Fraction failed in support: " + str(float(samples_failed_in_support) / opt.misc.samples)
