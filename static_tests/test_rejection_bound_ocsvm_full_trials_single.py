"""
This file is the same as test_rejection_bound_ocsvm_simple_full_trials_single.py
but it is using the regular oc svms rather than the reduced state space ones.
"""
...
def run_trial(opt):
    ocs = [svm.OneClassSVM(kernel='rbf', gamma = opt.gamma, nu = opt.nu) for t in range(opt.t)]
    ...
    for i in range(100):
        ...
        pickle.dump(trials_data, open(opt.data_dir + 'full/trials_data.pkl', 'w'))
    ...
    # rand_scores = np.zeros((opt.misc.samples / 2, max_rec + 1))
    # rand_cutoffs = np.zeros((opt.misc.samples / 2, max_rec + 1))
    # rand_completed = 0
    # rand_failed = 0
    # rand_failed_in_support = 0
    # for i in range(opt.misc.samples / 2):
    #     print "Eval Iteration: " + str(i) + ""
    #     triggered = False
    #     k = 0
    #     while not triggered:
    ...
    # approx_grad_scores = np.zeros((opt.misc.samples / 2, max_rec + 1))
    # approx_grad_cutoffs = np.zeros((opt.misc.samples / 2, max_rec + 1))
    # approx_grad_completed = 0
    # approx_grad_failed = 0
    # approx_grad_failed_in_support = 0
    # # for i in range(opt.misc.samples):
    # for i in range(opt.misc.samples / 2):
    #     print "Eval Iteration: " + str(i) + ""
    #     triggered = False
    ...
    #     approx_grad_completed += int(results[-3]['completed'])
    #     approx_grad_failed += int(results[-3]['failed'])
    #     approx_grad_failed_in_support += int(results[-3]['failed_in_support'])



    # print "\n\nFinite Diff Controls\n\n"


    # finite_diff_scores = np.zeros((1, max_rec + 1))
    # finite_diff_cutoffs = np.zeros((1, max_rec + 1))
    # finite_diff_completed = 0
    # finite_diff_failed = 0
    # finite_diff_failed_in_support = 0
    # for i in range(1):
    #     print "Eval Iteration: " + str(i) + ""
    #     triggered = False
    #     while not triggered:
    #         print "\t\tNot yet triggered"
    #         results = rec_statistics.collect_rec_finite_diff(opt.env, opt.sim, lnr, ocs, opt.t, opt, max_KLs, visualize=False, early_stop=False, init_state=init_state, max_rec=max_rec)
    #         triggered = results[-3]['triggered']
    #     finite_diff_scores[i, :] = results[-3]['rec_scores']
    #     finite_diff_cutoffs[i, :] = results[-3]['rec_cutoffs']
    #     finite_diff_completed += int(results[-3]['completed'])
    #     finite_diff_failed += int(results[-3]['failed'])
    #     finite_diff_failed_in_support += int(results[-3]['failed_in_support'])
    ...
    #     'approx_grad_tallies': [approx_grad_completed, approx_grad_failed, approx_grad_failed_in_support],
    #     'finite_diff_scores': finite_diff_scores,
    #     'finite_diff_cutoffs': finite_diff_cutoffs,
    #     'finite_diff_tallies': [finite_diff_completed, finite_diff_failed, finite_diff_failed_in_support],
    # }
