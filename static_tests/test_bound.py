...
def run_trial(opt):
    ...
    for j in range(num_samples):
        ...
        while reject:
            ...
            failed = results[-2]
            states = results[0]
            int_actions = results[1]
            if reject:
                print "\tRejecting " + str(j) + " and restarting..."

        trajs_lnr.append(states)
        actions_lnr.append(int_actions)

        if failed:
            lnr_failures += 1
            print "\t" + str(j) + " failed..."
    print "fraction of failures: " + str(lnr_failures / float(num_samples))
    ...
    max_Kt = -1
    max_Lt = -1
    for traj_states, traj_actions in zip(trajs_lnr, actions_lnr):
        zipped = zip(traj_states, traj_actions)
        for t, (state, action) in enumerate(zipped[:-1]):
            state_next, action_next = zipped[t+1]
            state_norm = np.linalg.norm(state_next - state)
    ...
    print "Max lnr L: " + str(max_L)
    print "Avg lnr L: " + str(np.mean(Ls))
    print "Max lnr K: " + str(max_K)
    print "Avg lnr K: " + str(np.mean(Ks))
