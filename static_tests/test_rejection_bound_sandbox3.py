"""
Using only 1 one class svm
Did not work, please ignore
"""
...
def main():
    ...
    plt.bar(index + 1 * width, lnr_means, width, label='no recovery', yerr=lnr_sems)
    plt.legend()
...
def run_trial(opt):
    ...
    for i, (traj_states, traj_actions) in enumerate(zip(trajs_train, actions_train)):
        ...
        for t, (state, action) in enumerate(zipped[:-1]):
            ...
            func_diff = np.abs(oc.decision_function([state])[0,0] - oc.decision_function([state_next])[0,0])
    ...
    for j in range(opt.misc.samples):
        ...
        reject = True
        while reject:
            results = statistics.collect_robust_traj_multiple_rejection_adaptive(opt.env, lnr, oc, opt.t, opt, max_KLs, False, False)
            reject = results[-1]
            failed = results[-2]
            if reject:
    ...
    for j in range(opt.misc.samples):
        ...
        reject = True
        while reject:
            results = statistics.collect_score_traj_multiple_rejection(opt.env, lnr, oc, opt.t, False, False)
            reject = results[-1]
            failed = results[-2]
            info = results[-3]
            if reject:
