...
from icra_tools.ocsvm2 import OCSVM
import argparse
...
def main():
    ...
    for t in range(opt.trials):
        ...
        rec_mo_scores += info['rec_mo_scores'] 
        rec_mo_cutoffs += info['rec_mo_cutoffs']
        ...
        pickle.dump(rec_mo_scores, open(opt.data_dir + 'multiple_trials/rec_mo_scores.pkl', 'w'))
        pickle.dump(rec_mo_cutoffs, open(opt.data_dir + 'multiple_trials/rec_mo_cutoffs.pkl', 'w'))
...
def run_trial(opt):
    ...
    for k in range(20):
        ...
        rand_mo_completed = 0
        rand_mo_failed = 0
        rand_mo_failed_in_support = 0

        for i in range(opt.misc.samples):
            print "Eval Iteration: " + str(i)
            results = rec_statistics.collect_rec(rec_statistics.finite_diff_loop, opt.env, opt.sim, lnr, ocs, opt.t, opt, max_KLs, visualize=False, early_stop=False, init_state=init_state, max_rec=max_rec)
            rand_mo_completed += int(results[-3]['completed'])
            rand_mo_failed += int(results[-3]['failed'])
            rand_mo_failed_in_support += int(results[-3]['failed_in_support'])

        rand_mo_completed = rand_mo_completed / float(opt.misc.samples)
        rand_mo_failed = rand_mo_failed / float(opt.misc.samples)
        rand_mo_failed_in_support = rand_mo_failed_in_support / float(opt.misc.samples)
        rec_mo_scores += results[-3]['rec_scores']
        rec_mo_cutoffs += results[-3]['rec_cutoffs']
        ...
        results = {
            ...
            'rand_mo_tallies': [rand_mo_completed, rand_mo_failed, rand_mo_failed_in_support],
        }
        ...
        pickle.dump(rec_mo_scores, open(opt.data_dir + 'multiple_trials/rec_mo_scores' + str(opt.t_value) + '.pkl', 'w'))
        pickle.dump(rec_mo_cutoffs, open(opt.data_dir + 'multiple_trials/rec_mo_cutoffs' + str(opt.t_value) + '.pkl', 'w'))


    info = {'rec_scores': rec_scores, 'rec_cutoffs': rec_cutoffs,
            'rec_mo_scores': rec_mo_scores, 'rec_mo_cutoffs': rec_mo_cutoffs}
